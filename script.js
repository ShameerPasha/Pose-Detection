...
let URL = "https://teachablemachine.withgoogle.com/models/i0X6YntJi/";
...
let backgroundSong = new Audio('backgroundSong.mp3');
...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time <= 2.5 && !poseState.triggered) {
            ...
            case '4':
                ...
                break;
            case '5':
                if (time >= 9.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
...
async function playInstructionVideo() {
    ...
    const videoCanvas = document.getElementById('poseCanvas')
